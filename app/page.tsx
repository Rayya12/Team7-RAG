...
// (Optional) If you have tailwind-typography, these classes will apply nicely.
// Otherwise the base Tailwind utilities still look fine.

function MarkdownMessage({ text }: { text: string }) {
  return (
    <div className="prose prose-sm max-w-none dark:prose-invert prose-p:my-2 prose-ul:my-2 prose-ol:my-2 prose-li:my-1 prose-pre:my-2">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        // Disallow raw HTML for safety; we only render markdown
        ...
        components={{
          ...
          code: ({ inline, className, children, ...props }:any) => (
            inline ? (
              <code
                className="px-1.5 py-0.5 rounded-md bg-black/5 dark:bg-white/10 text-[0.9em] break-words"
                {...props}
              >
                {children}
              </code>
            ) : (
              <pre className="rounded-xl p-3 bg-black/90 text-white overflow-x-auto text-sm">
                <code>{children}</code>
              </pre>
            )
          ),
          ...
          h1: ({ children, ...props }) => (
            <h1 className="text-xl font-semibold mb-1" {...props}>{children}</h1>
          ),
          h2: ({ children, ...props }) => (
            <h2 className="text-lg font-semibold mb-1" {...props}>{children}</h2>
          ),
          h3: ({ children, ...props }) => (
            <h3 className="text-base font-semibold mb-1" {...props}>{children}</h3>
          ),
          ...
          th: ({ children, ...props }) => (
            <th className="border px-2 py-1 text-left bg-black/5" {...props}>{children}</th>
          ),
          td: ({ children, ...props }) => (
            <td className="border px-2 py-1 align-top" {...props}>{children}</td>
          ),
          img: ({ src, alt }) => (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full px-4 md:px-0">
              <button
                ...
                className="bg-white p-4 rounded-lg shadow-md text-left transition-transform hover:scale-[1.02]"
              >
                <div className="w-10 h-10 mb-2 bg-blue-100 rounded-md grid place-items-center">
              ...
              <button
                ...
                className="bg-white p-4 rounded-lg shadow-md text-left transition-transform hover:scale-[1.02]"
              >
                <div className="w-10 h-10 mb-2 bg-pink-100 rounded-md grid place-items-center">
              ...
              <button
                ...
                className="bg-white p-4 rounded-lg shadow-md text-left transition-transform hover:scale-[1.02]"
              >
                <div className="w-10 h-10 mb-2 bg-yellow-100 rounded-md grid place-items-center">
          ...
          messages.map((m) => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className="flex items-start gap-3 max-w-[80%]">
                <div
                  className={`p-4 rounded-2xl shadow-sm break-words ${
                    m.role === "user"
                      ? "bg-[#3B82F6] text-white rounded-br-md ml-auto"
                      : "bg-white text-gray-800 rounded-bl-md"
                  }`}
                >
                  {m.parts?.map((part: any, idx: number) => {
                    if (part.type === "text") {
                      return <MarkdownMessage key={idx} text={part.text} />;
                    }
                    return null;
                  })}
                </div>
